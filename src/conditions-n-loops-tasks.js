...
function convertNumberToString(numberStr) {
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    ...
    if (numberStr[i] === '-') s += 'minus';
    if (numberStr[i] === '.' || numberStr[i] === ',') s += 'point';
...
function getBalanceIndex(arr) {
  ...
  for (let i = 1; i < arr.length; i += 1) {
    ...
    for (let r = arr.length - 1; r >= i + 1; r -= 1) {
      s2 += arr[r];
    }
    if (s1 === s2) {
      return i;
...
function rotateMatrix(matrix) {
  ...
  for (i = 0; i < matrix.length; i += 1) {
    for (j = 0; j < matrix[0].length; j += 1) {
      temp[j][i] = matrix[matrix[i].length - 1 - i][j];
      // matrix[j][i] = temp[temp[i].length - 1 - i][j];
    }
  }
  console.log(temp);
  return temp;
...
function sortByAsc(array) {
  ...
  return [...sortByAsc(left), base, ...sortByAsc(right)];
}
